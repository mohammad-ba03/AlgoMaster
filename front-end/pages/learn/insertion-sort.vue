    ...
    try {
        ...
        function checkAnswer(event, isCorrect) {
            ...
            if(isCorrect) {
                ...
                siblings.forEach(b => {
                    if(b.textContent.includes('Index 0') || b.textContent.includes('Shift sorted') || b.textContent.includes('O(N)')) {
        ...
        function initVisual() {
            ...
            currentArray.forEach((num, index) => {
                ...
                // First element starts as "sorted" visually
                if (index === 0) box.classList.add('sorted-zone');
...
async function saveAndExit() {
    ...
    try {
        ...
        if (data.helpPointsAwarded > 0) {
            ...
            setTimeout(() => {
                navigateTo('/learn/recursion');
            }, 2000);
           
        } else{
        navigateTo('/learn/recursion'); }// استخدام دالة Nuxt للتوجيه بدلاً من window.location
        ...
        async function runSimulation() {
            ...
            const logicCond = document.getElementById('code-cond').value;
            const logicShift = document.getElementById('code-shift').value;
            
            if(!logicCond || !logicShift) {
                alert("Please fill in the blanks!");
                return;
            }

            isRunning = true;
            document.getElementById('status-msg').innerText = "Sorting Cards...";
            
            let n = currentArray.length;

            // Loop starting from 1 (Index 0 is considered sorted)
            for (let i = 1; i < n; i++) {
                if(!isRunning) break;
                ...
                document.getElementById('status-msg').innerText = `Picked Card: ${key}`;
                await new Promise(r => setTimeout(r, 800));

                // Inner While Loop (Shifting)
                while (j >= 0) {
                    if(!isRunning) break;
                    ...
                    // --- EVALUATE LOGIC ---
                    if (logicCond === "correct") {
                        if (currentArray[j] > key) shouldShift = true;
                    } else if (logicCond === "wrong_less") {
                        // Reverse Sort
                        if (currentArray[j] < key) shouldShift = true;
                    }

                    if (shouldShift) {
                        document.getElementById('status-msg').innerText = `${currentArray[j]} > ${key}. Shifting ${currentArray[j]} Right.`;
                        
                        // Visual Shift: We need to move content. 
                        // To animate, we make current J look like it's sliding to J+1
                        const boxJ = document.getElementById(`box-${j}`);
                        const boxNext = document.getElementById(`box-${j+1}`);
                        
                        // Simple Animation: Change Text
                        if (logicShift === "correct") {
                            // Update Data
                            currentArray[j + 1] = currentArray[j];
                            // Update Visual
                            boxNext.innerHTML = `${currentArray[j]} <span class="index-label">${j+1}</span>`;
                            boxNext.classList.add('sorted-zone'); // Inherit sorted color
                            await new Promise(r => setTimeout(r, 600));
                        } else {
                            // Wrong Logic: Don't shift properly or shift 0/key
                            document.getElementById('status-msg').innerHTML = `<span style="color:#ef4444">Bug! Logic Error in Shifting. Data Corrupted.</span>`;
                ...
                if (logicShift === "correct") {
                    ...
                    const targetBox = document.getElementById(`box-${j+1}`);
                    targetBox.innerHTML = `${key} <span class="index-label">${j+1}</span>`;
                    
                    // Drop Animation
                    keyBox.classList.remove('active-key'); // Remove from old pos visually (logic handled by overwrite)
                    // But visually, the "old" key position might be overwritten or duplicate.
                    // In Insertion Sort visualizers, typically the 'hole' moves. 
                    // Here we simply update the DOM text at the end.
                    
                    // Visual Cleanup: Ensure all up to I are green
                    for(let k=0; k<=i; k++) {
                        document.getElementById(`box-${k}`).classList.add('sorted-zone');
                        // Fix content just in case
                        document.getElementById(`box-${k}`).innerHTML = `${currentArray[k]} <span class="index-label">${k}</span>`;
                    }
                    
                    document.getElementById(`box-${j+1}`).classList.add('inserted');
                    await new Promise(r => setTimeout(r, 600));
                    document.getElementById(`box-${j+1}`).classList.remove('inserted');
                }
            }

            if(isRunning) {
                document.getElementById('status-msg').innerHTML = `<span style="color:#22c55e"><i class="fas fa-check"></i> Hand Sorted!</span>`;
                sessionPoints += 40;
...
<style>
        ...
        /* --- CARD VISUALIZER STYLES --- */
        .array-container { display: flex; gap: 10px; margin-bottom: 40px; flex-wrap: wrap; justify-content: center; min-height: 120px; padding-top: 60px; }
        
        .array-box { 
            width: 50px; height: 75px; /* Rectangular Card Shape */
            border: 2px solid #475569; border-radius: 6px;
            background: #fff; color: #0f172a; /* White Card Look */
        ...
        /* Playing Card decorative corners */
        .array-box::before { content: '♠'; position: absolute; top: 2px; left: 4px; font-size: 0.7rem; color: #334155; }
        .array-box::after { content: '♠'; position: absolute; bottom: 2px; right: 4px; font-size: 0.7rem; transform: rotate(180deg); color: #334155; }

        /* States */
        .array-box.sorted-zone { border-color: #22c55e; background: #dcfce7; } /* Light Green */
        .array-box.active-key { 
            background: #facc15; border-color: #ca8a04; 
            transform: translateY(-50px) scale(1.1); /* Lift Up Effect */
            z-index: 10; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.5);
        }
        .array-box.shifting { transform: translateX(100%); opacity: 0.7; } /* Moving Right */
        .array-box.inserted { animation: dropDown 0.5s ease-out; background: #22c55e; color: white; border-color: #22c55e; }

        .index-label { position: absolute; bottom: -25px; font-size: 0.7rem; color: #94a3b8; font-weight: normal; }
        ...
        @keyframes dropDown { from { transform: translateY(-50px); opacity: 0.5; } to { transform: translateY(0); opacity: 1; } }
...
<nav class="navbar">
    ...
    <div class="lesson-wrapper">
        ...
        <div id="step-0" class="section-view active">
            <div class="quiz-container" style="text-align: center;">
                <h1 class="gradient-text">Insertion Sort</h1>
                <p>Sort like you sort playing cards in your hand.</p>
                <div style="margin: 40px 0;">
                    <i class="fas fa-hand-paper" style="font-size: 4rem; color: #3b82f6; margin-bottom: 20px;"></i>
                    <p class="text-muted">Pick a card, shift the larger ones to the right, and insert the card in its correct spot.</p>
                </div>
        ...
        <div id="step-1" class="section-view">
            <div class="quiz-container">
                ...
                <p style="margin-bottom: 20px;">Understand the mechanics.</p>
                
                <div class="question-card">
                    <h4>1. Which part of the array is "Sorted" initially?</h4>
                    <div class="options-grid">
                        <button class="option-btn" @click="checkAnswer($event, false)">The whole array</button>
                        <button class="option-btn" @click="checkAnswer($event, true)">Just the first element (Index 0)</button>
                        <button class="option-btn" @click="checkAnswer($event, false)">The last element</button>
                    </div>
                </div>

                <div class="question-card">
                    <h4>2. What happens if the card in hand is smaller than the sorted ones?</h4>
                    <div class="options-grid">
                        <button class="option-btn" @click="checkAnswer($event, true)">Shift sorted items to the Right</button>
                        <button class="option-btn" @click="checkAnswer($event, false)">Swap them immediately</button>
                        <button class="option-btn" @click="checkAnswer($event, false)">Delete the card</button>
                    </div>
                </div>

                <div class="question-card">
                    <h4>3. What is the Best Case time complexity (Already sorted)?</h4>
                    <div class="options-grid">
                        ...
                        <button class="option-btn" @click="checkAnswer($event, true)">O(N) - Very Fast</button>
                    </div>
                </div>
        ...
        <div id="step-2" class="section-view">
            <div class="practice-layout">
                ...
                <div class="code-panel" style="background: #1e1e1e; padding: 20px; border-radius: 12px; border: 1px solid #333;">
                    <h4 style="margin-bottom: 15px;"><i class="fas fa-laptop-code"></i> Implement Logic</h4>
                    
                    <div class="code-editor" style="font-family: 'Consolas', monospace; color: #d4d4d4; line-height: 1.8;">
                        <div class="code-line"><span style="color:#c586c0">for</span> (<span style="color:#569cd6">let</span> i = 1; i < n; i++) {</div>
                        <div class="code-line"> &nbsp; <span style="color:#569cd6">let</span> key = arr[i]; <span style="color:#6a9955">// Pick Card</span></div>
                        <div class="code-line"> &nbsp; <span style="color:#569cd6">let</span> j = i - 1;</div>
                        <div class="code-line"> &nbsp; <span style="color:#6a9955">// Shift larger elements right</span></div>
                        <div class="code-line"> &nbsp; <span style="color:#c586c0">while</span> (j >= 0 && <select id="code-cond" class="code-select">
                            <option value="" disabled selected>Condition?</option>
                            <option value="correct">arr[j] > key</option>
                            <option value="wrong_less">arr[j] < key</option>
                            <option value="wrong_eq">arr[j] == key</option>
                        </select>) {</div>
                        <div class="code-line"> &nbsp; &nbsp; arr[j + 1] = <select id="code-shift" class="code-select">
                            <option value="" disabled selected>Action?</option>
                            <option value="wrong_key">key</option>
                            <option value="correct">arr[j]</option>
                            <option value="wrong_zero">0</option>
                        </select>; <span style="color:#6a9955">// Shift Right</span></div>
                        <div class="code-line"> &nbsp; &nbsp; j = j - 1;</div>
                        <div class="code-line"> &nbsp; }</div>
                        <div class="code-line"> &nbsp; arr[j + 1] = key; <span style="color:#6a9955">// Insert Key</span></div>
                        <div class="code-line">}</div>
                    </div>
    ...
    <div class="info-modal" id="info-modal">
        <div class="info-content">
            <i class="fas fa-hand-holding fa-3x" style="color:var(--primary); margin-bottom: 15px;"></i>
            <h3>How Insertion Sort Works</h3>
            <p style="color:var(--text-muted); margin: 15px 0;">
                1. <strong>Pick</strong> the next card (Key).<br>
                2. <strong>Compare</strong> it backwards with the sorted cards.<br>
                3. <strong>Shift</strong> larger cards to the right.<br>
                4. <strong>Insert</strong> the Key into the empty spot.
            </p>
    ...
    <div class="modal-overlay" id="success-modal">
        <div class="modal-content">
            <div class="modal-header gradient-text">Excellent!</div>
            <p>You sorted the hand perfectly.</p>
