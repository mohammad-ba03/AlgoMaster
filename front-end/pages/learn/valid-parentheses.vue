...
// --- State Management ---
        ...
        // Test String: Valid Complex
        const testString = "{ ( [ ] ) }"; 
        // Note: You can change this to "{(])}" to test invalid visual logic manually
    ...
    try {
        ...
        function checkAnswer(event, isCorrect) {
            ...
            if(isCorrect) {
                ...
                siblings.forEach(b => {
                    if(b.textContent.includes('First Out') || b.textContent.includes('Push') || b.textContent.includes('Mismatch') || b.textContent.includes('No (Unclosed')) {
        ...
        // --- Visualizer Logic ---
        function initVisual() {
            const strContainer = document.getElementById('visual-string');
            strContainer.innerHTML = '';
            const stackContainer = document.getElementById('visual-stack');
            stackContainer.innerHTML = '<div class="stack-label">STACK</div>'; // Clear but keep label
...
async function saveAndExit() {
    ...
    try {
        ...
        if (data.helpPointsAwarded > 0) {
            ...
            setTimeout(() => {
                navigateTo('/learn/simple-queue');
            }, 2000);
           
        } else{
        navigateTo('/learn/simple-queue'); }// استخدام دالة Nuxt للتوجيه بدلاً من window.location
        ...
        async function runSimulation() {
            ...
            const logicPush = document.getElementById('code-push').value;
            const logicFinal = document.getElementById('code-final').value;
            
            if(!logicPush || !logicFinal) {
                alert("Please complete the code!");
                return;
            }

            isRunning = true;
            document.getElementById('status-msg').innerText = "Analyzing string...";
            ...
            for (let i = 0; i < strBoxes.length; i++) {
                if(!isRunning) break;

                const box = strBoxes[i];
                ...
                if (isOpen(char)) {
                    // --- OPEN BRACKET ---
                    document.getElementById('status-msg').innerText = `Open bracket '${char}'. Push to stack.`;
                    
                    if (logicPush === "correct") {
                        // Correct Push Logic
                        stackData.push(char);
                        ...
                        item.innerText = char;
                        stackContainer.appendChild(item); // Adds to top (flex-reverse makes it visual bottom? No, flex-reverse starts from bottom. Standard append works for visual "top" if styled right. Wait, flex-col-reverse means first child is at bottom. So appendChild adds to visual TOP. Correct.)
                        
                        await new Promise(r => setTimeout(r, 600));

                    } else if (logicPush === "wrong_pop") {
                        document.getElementById('status-msg').innerHTML = `<span style="color:#ef4444">Bug! Trying to POP an open bracket? Empty stack crash!</span>`;
                        errorFound = true; break;
                    }
                    
                } else {
                    // --- CLOSED BRACKET ---
                    const needed = getMatch(char);
                    document.getElementById('status-msg').innerText = `Closed '${char}'. Check if top is '${needed}'...`;
                    await new Promise(r => setTimeout(r, 800));

                    // Check top of visual stack
                    const visualItems = document.querySelectorAll('.stack-item');
                    const topItem = visualItems[visualItems.length - 1];

                    if (stackData.length === 0 || stackData[stackData.length-1] !== needed) {
                        // Mismatch or Empty
                        document.getElementById('status-msg').innerHTML = `<span style="color:#ef4444">Mismatch! Expected '${needed}' but found '${stackData[stackData.length-1] || "Empty"}'. Invalid!</span>`;
                        if(topItem) topItem.classList.add('error');
                        box.classList.add('error'); // Make char red
                        errorFound = true;
                        break;
                    } else {
                        // Match Found
                        document.getElementById('status-msg').innerHTML = `<span style="color:#22c55e">Match found! Popping '${needed}'.</span>`;
                        
                        // Logic Pop
                        stackData.pop();
                        
                        // Visual Pop
                        topItem.classList.add('pop');
                        await new Promise(r => setTimeout(r, 600));
                        topItem.remove();
                    }
                }
            ...
            // Final Check
            if (!errorFound) {
                if (logicFinal === "correct") {
                    if (stackData.length === 0) {
                        document.getElementById('status-msg').innerHTML = `<span style="color:#22c55e"><i class="fas fa-check"></i> Stack Empty. Valid Code!</span>`;
                        sessionPoints += 50;
                        setTimeout(showSuccess, 1000);
                    } else {
                        document.getElementById('status-msg').innerHTML = `<span style="color:#ef4444">Stack NOT empty. Unclosed brackets remain. Invalid.</span>`;
                    }
                } else {
                    // Wrong return logic (e.g. return true always)
                    document.getElementById('status-msg').innerHTML = `<span style="color:#eab308">Simulation done, but your code returns '${logicFinal}' blindly! Logic Error.</span>`;
                }
            }
...
<style>
        ...
        .visual-panel { background: #0f172a; padding: 20px; border-radius: 12px; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; min-height: 450px; border: 1px solid var(--border-light); position: relative; }
        
        /* --- STACK VISUALIZER STYLES --- */
        
        /* The Code String at Top */
        .string-container { display: flex; gap: 5px; margin-bottom: 40px; justify-content: center; padding-top: 20px;}
        .char-box { 
            width: 40px; height: 40px; border: 1px solid #475569; border-radius: 5px;
            display: flex; align-items: center; justify-content: center;
            font-family: monospace; font-size: 1.2rem; background: #1e293b; color: #fff;
            transition: 0.3s;
        }
        .char-box.current { border-color: #eab308; background: rgba(234, 179, 8, 0.2); transform: translateY(-5px); box-shadow: 0 5px 10px rgba(234, 179, 8, 0.3); }
        .char-box.processed { opacity: 0.3; }

        /* The Stack Container (Bucket) */
        .stack-bucket {
            width: 100px;
            height: 250px;
            ...
            display: flex;
            flex-direction: column-reverse; /* Items stack from bottom */
            align-items: center;
            padding: 10px;
            gap: 5px;
            position: relative;
        }
        .stack-label { position: absolute; bottom: -30px; color: #94a3b8; font-size: 0.9rem; }
        ...
        .stack-item.pop { animation: popOut 0.5s forwards; background: #22c55e; } /* Green on pop match */
        .stack-item.error { background: #ef4444; animation: shake 0.4s; }
...
<template>
    ...
    <div class="lesson-wrapper">
        ...
        <div id="step-0" class="section-view active">
            <div class="quiz-container" style="text-align: center;">
                <h1 class="gradient-text">Valid Parentheses</h1>
                <p>Using a Stack to validate code structure.</p>
                <div style="margin: 40px 0;">
                    <i class="fas fa-layer-group" style="font-size: 4rem; color: #3b82f6; margin-bottom: 20px;"></i>
                    <p class="text-muted">LIFO (Last-In, First-Out). Push open brackets, Pop closed brackets.</p>
                </div>
        ...
        <div id="step-1" class="section-view">
            <div class="quiz-container">
                ...
                <p style="margin-bottom: 20px;">Master the Stack Concept.</p>
                
                <div class="question-card">
                    <h4>1. What does LIFO stand for?</h4>
                    <div class="options-grid">
                        <button class="option-btn" @click="checkAnswer($event, false)">Last In, First On</button>
                        <button class="option-btn" @click="checkAnswer($event, true)">Last In, First Out</button>
                        <button class="option-btn" @click="checkAnswer($event, false)">Low Input, Fast Output</button>
                    </div>
                </div>

                <div class="question-card">
                    <h4>2. Which operation adds an item to the Stack?</h4>
                    <div class="options-grid">
                        <button class="option-btn" @click="checkAnswer($event, true)">Push</button>
                        <button class="option-btn" @click="checkAnswer($event, false)">Pop</button>
                        <button class="option-btn" @click="checkAnswer($event, false)">Peek</button>
                    </div>
                </div>

                <div class="question-card">
                    <h4>3. Is this string valid: "([)]"?</h4>
                    <div class="options-grid">
                        <button class="option-btn" @click="checkAnswer($event, false)">Yes</button>
                        <button class="option-btn" @click="checkAnswer($event, true)">No (Mismatch order)</button>
                        <button class="option-btn" @click="checkAnswer($event, false)">Maybe</button>
                    </div>
                </div>

                <div class="question-card">
                    <h4>4. If the stack is NOT empty at the end, is it valid?</h4>
                    <div class="options-grid">
                        <button class="option-btn" @click="checkAnswer($event, true)">No (Unclosed brackets exist)</button>
                        <button class="option-btn" @click="checkAnswer($event, false)">Yes (Ignore them)</button>
                    </div>
                </div>
        ...
        <div id="step-2" class="section-view">
            <div class="practice-layout">
                ...
                <div class="visual-panel">
                    <div class="string-container" id="visual-string"></div>
                    
                    <div class="stack-bucket" id="visual-stack">
                ...
                <div class="code-panel" style="background: #1e1e1e; padding: 20px; border-radius: 12px; border: 1px solid #333;">
                    <h4 style="margin-bottom: 15px;"><i class="fas fa-laptop-code"></i> Code the Logic</h4>
                    
                    <div class="code-editor" style="font-family: 'Consolas', monospace; color: #d4d4d4; line-height: 1.8;">
                        <div class="code-line"><span style="color:#c586c0">function</span> isValid(s) {</div>
                        <div class="code-line"> &nbsp; <span style="color:#569cd6">let</span> stack = [];</div>
                        <div class="code-line"> &nbsp; <span style="color:#c586c0">for</span> (<span style="color:#569cd6">let</span> char <span style="color:#c586c0">of</span> s) {</div>
                        <div class="code-line"> &nbsp; &nbsp; <span style="color:#c586c0">if</span> (isOpen(char)) {</div>
                        <div class="code-line"> &nbsp; &nbsp; &nbsp; <span style="color:#6a9955">// Add to stack</span></div>
                        <div class="code-line"> &nbsp; &nbsp; &nbsp; <select id="code-push" class="code-select">
                            <option value="" disabled selected>Action?</option>
                            <option value="correct">stack.push(char);</option>
                            <option value="wrong_pop">stack.pop();</option>
                            <option value="wrong_len">stack.length++;</option>
                        </select></div>
                        <div class="code-line"> &nbsp; &nbsp; } <span style="color:#c586c0">else</span> {</div>
                        <div class="code-line"> &nbsp; &nbsp; &nbsp; <span style="color:#6a9955">// If matching pair...</span></div>
                        <div class="code-line"> &nbsp; &nbsp; &nbsp; <span style="color:#c586c0">if</span> (stack.pop() !== match(char)) <span style="color:#c586c0">return</span> <span style="color:#569cd6">false</span>;</div>
                        <div class="code-line"> &nbsp; &nbsp; }</div>
                        <div class="code-line"> &nbsp; }</div>
                        <div class="code-line"> &nbsp; <span style="color:#6a9955">// Final check</span></div>
                        <div class="code-line"> &nbsp; <span style="color:#c586c0">return</span> <select id="code-final" class="code-select">
                            <option value="" disabled selected>Check?</option>
                            <option value="wrong_true">true</option>
                            <option value="correct">stack.length === 0</option>
                            <option value="wrong_null">stack === null</option>
                        </select>;</div>
                        <div class="code-line">}</div>
                    </div>
    ...
    <div class="info-modal" id="info-modal">
        <div class="info-content">
            <i class="fas fa-layer-group fa-3x" style="color:var(--primary); margin-bottom: 15px;"></i>
            <h3>The Stack Rule</h3>
            <p style="color:var(--text-muted); margin: 15px 0;">
                Imagine a box of Pringles. The last chip you put in is the first one you eat.
                <br><br>
                1. <strong>Push</strong>: Add open bracket `(` to top.<br>
                2. <strong>Pop</strong>: When `)` comes, remove the top `(`.
                <br>
                If the box isn't empty at the end, or you try to pop from an empty box, the code is invalid.
            </p>
    ...
    <div class="modal-overlay" id="success-modal">
        <div class="modal-content">
            <div class="modal-header gradient-text">Level Up!</div>
            <p>You mastered the Stack Data Structure.</p>
